...
const coronationElsaPlannerNowMs = () => (
...
const buildStrongestModeCoronationElsaFlowSupportStates = (game, physicsBodies = null) => {
  ...
  const bodies = (physicsBodies || game.getPhysicsBodies?.() || game.tsums || []).filter((body) => (
    body &&
    !body.dead &&
    (!body.removing || body.clearOccupying) &&
  ...
  for (const body of bodies) {
    ...
    for (const other of bodies) {
      ...
      if (Math.hypot(dx, dy) <= minDistance + 1.5) {
        supportBodiesByBody.get(body).push(other);
      }
    }
  }
  const stableMemo = new Map();
  const hasStableSupport = (body) => {
    if (stableMemo.has(body)) return stableMemo.get(body);
    const stableBase = !!(
      floorSupportedBodies.has(body) ||
      game.boardState?.isFrozen?.(body) ||
      game.isBodyMotionLocked?.(body) ||
      game.isBodySettled?.(body)
    );
    if (stableBase) {
      stableMemo.set(body, true);
      return true;
    }
    stableMemo.set(body, false);
    const stable = (supportBodiesByBody.get(body) || []).some((support) => hasStableSupport(support));
    stableMemo.set(body, stable);
    return stable;
  };
  const supportStateByBody = new Map();
  for (const body of bodies) {
    const supportBodies = supportBodiesByBody.get(body) || [];
    const stableSupport = hasStableSupport(body);
    const dynamicSupport = !stableSupport && supportBodies.length > 0;
    const genuineFallSpace = !stableSupport && supportBodies.length === 0;
    supportStateByBody.set(body, Object.freeze({
      supportKind: stableSupport ? "stable" : (dynamicSupport ? "dynamic" : "fall-space"),
      stableSupport,
      dynamicSupport,
      genuineFallSpace,
      supportCount: supportBodies.length
    }));
  }
  return supportStateByBody;
};
...
const classifyStrongestModeCoronationElsaFlowSafety = (game, tsum, context = null) => {
  ...
  const supportStateByBody = context?.supportStateByBody || buildStrongestModeCoronationElsaFlowSupportStates(
    game,
    context?.physicsBodies
  );
  const supportState = supportStateByBody.get(tsum) || Object.freeze({
  ...
  const structurallyFlowing = supportState.dynamicSupport || supportState.genuineFallSpace;
  const activeInflow = !unavailable && !settled && structurallyFlowing;
  const inflowUnsafe = activeInflow;
  return Object.freeze({
    ...
    moving,
    upper,
    supportKind: supportState.supportKind,
    stableSupport: supportState.stableSupport,
    dynamicSupport: supportState.dynamicSupport,
    genuineFallSpace: supportState.genuineFallSpace,
    naturalFallSpace: supportState.genuineFallSpace,
    supportCount: supportState.supportCount,
...
class BoardStateService {
  ...
  reset() {
    ...
    this._nextGroupCounter = 1;
    this._nextHandleCounter = 1;
    this.game.strongestModeCoronationElsaFreezeRevision = (
      this.game.strongestModeCoronationElsaFreezeRevision || 0
    ) + 1;
  }
  ...
  applyFreezeEntries(nodeIds, spec = {}, defaults = {}) {
    ...
    if (changed) this.game.strongestModeCoronationElsaFreezeRevision += 1;
    return groupId;
  }
  ...
  clearFrozenGroup(groupId) {
    ...
    if (nodes.length) this.game.strongestModeCoronationElsaFreezeRevision += 1;
    return nodes;
  }
...
class Game {
  constructor(canvas, options = {}) {
    ...
    this.gameMode = "solo";
    this.battleDifficulty = "strongest";
    ...
    this.strongestModeCoronationElsaSupportCache = null;
    this.strongestModeCoronationElsaPlannerFrameCallCount = 0;
    this.strongestModeCoronationElsaPlannerLastFrameCallCount = 0;
    this.strongestModeCoronationElsaPlannerLastTraceCallCount = 0;
    this.strongestModeCoronationElsaPlannerCallsSinceTrace = 0;
    this.strongestModeCoronationElsaPhysicsStepCount = 0;
    this.strongestModeCoronationElsaGameplayDeltaTotalSec = 0;
    this.strongestModeCoronationElsaPlannerBlockedTotalMs = 0;
    this.strongestModeCoronationElsaOpportunityPerfBaseline = null;
    ...
    this.coronationElsaPerf = debugQuery?.get("coronationElsaPerf") === "1";
    this.coronationElsaPerfTarget = Math.max(1, Number(debugQuery?.get("coronationElsaPerfRuns")) || 30);
    ...
    this.strongestAutoStartRequested = (
      (this.coronationElsaDebug || this.coronationElsaPerf) &&
...
// (skill helpers moved to the end of the file)
  ...
  startStrongestAutoStartIfRequested() {
    if (!this.strongestAutoStartRequested || (!this.coronationElsaDebug && !this.coronationElsaPerf)) {
      return;
    }
  ...
  getCoronationElsaPlannerCacheKeys() {
    ...
    const supportParts = [];
    for (const tsum of this.tsums) {
      const resolvedType = this.boardState.getResolvedType?.(tsum) || tsum?.type || null;
      const effectiveRadius = this.getBodyRadius?.(tsum)
        ?? this.boardState.getEffectiveRadius?.(tsum)
        ?? tsum?.radius
        ?? 0;
      ...
      const rule = baseTraceEligible ? this.getChainBehaviorForStart(tsum) : null;
      const ruleKey = rule?.allowedTypeIds?.size
        ? `${Array.from(rule.allowedTypeIds).sort().join(",")}:${rule.unlimitedDistance ? 1 : 0}`
        : "-";
      const geometry = [
        tsum?.id,
        tsum?.x,
      ...
      supportParts.push([
        ...
        tsum?.vx,
        tsum?.vy,
        this.boardState.isFrozen?.(tsum) ? 1 : 0,
        this.isBodyMotionLocked?.(tsum) ? 1 : 0,
        this.isBodySettled?.(tsum) ? 1 : 0,
        tsum?.clearOccupying ? 1 : 0
      ].join(":"));
    }
    return Object.freeze({
      geometryKey: geometryParts.join("\u001d"),
      supportKey: supportParts.join("\u001d"),
      frameRevision: this.strongestModeCoronationElsaPlannerFrameRevision || 0,
      freezeRevision: this.strongestModeCoronationElsaFreezeRevision || 0
    });
  }

  isCoronationElsaPlannerRevisionCurrent(revision) {
    return !!revision && (
      revision.frameRevision === (this.strongestModeCoronationElsaPlannerFrameRevision || 0) &&
      revision.freezeRevision === (this.strongestModeCoronationElsaFreezeRevision || 0)
    );
  }
  ...
  buildCoronationElsaPlannerContext(options = {}) {
    ...
    const clock = typeof options.now === "function" ? options.now : coronationElsaPlannerNowMs;
    const deadlineMs = Number.isFinite(options.deadlineMs) ? options.deadlineMs : Number.POSITIVE_INFINITY;
    const keys = this.getCoronationElsaPlannerCacheKeys();
    const flowSupportStartedAt = clock();
    let flowContext = null;
    let supportCacheHit = false;
    if (this.strongestModeCoronationElsaSupportCache?.key === keys.supportKey) {
      flowContext = this.strongestModeCoronationElsaSupportCache.value;
      supportCacheHit = true;
    } else {
      flowContext = this.getStrongestModeCoronationElsaFlowSafetyContext();
      this.strongestModeCoronationElsaSupportCache = Object.freeze({
        key: keys.supportKey,
        value: flowContext
      });
    }
    const flowSupportBuildMs = clock() - flowSupportStartedAt;
    ...
    const snapshotBuildMs = clock() - snapshotStartedAt;
    const adjacencyStartedAt = clock();
    let adjacency = null;
    let adjacencyCacheHit = false;
    if (this.strongestModeCoronationElsaAdjacencyCache?.key === keys.geometryKey) {
      ...
      if (!adjacency.aborted) {
        this.strongestModeCoronationElsaAdjacencyCache = Object.freeze({
          key: keys.geometryKey,
          value: adjacency
        });
      }
    }
    const adjacencyBuildMs = clock() - adjacencyStartedAt;
    return Object.freeze({
      snapshot,
      adjacency,
      initialState: snapshot.initialState,
      revision: Object.freeze({
        frameRevision: keys.frameRevision,
        freezeRevision: keys.freezeRevision,
        geometryKey: keys.geometryKey,
        supportKey: keys.supportKey
      }),
      diagnostics: Object.freeze({
        ...
        supportCacheHit,
        adjacencyCacheHit,
        contextBudgetExceeded: adjacency.aborted || clock() >= deadlineMs
      })
    });
  }
  ...
  planStrongestModeCoronationElsaAction(options = {}) {
    ...
    const deadlineMs = Number.isFinite(options.deadlineMs)
      ? options.deadlineMs
      : startedAt + CORONATION_ELSA_PLANNER_CONFIG.hardBudgetMs;
    ...
    const solverDeadlineMs = Math.max(
      solveStartedAt,
      deadlineMs - CORONATION_ELSA_PLANNER_CONFIG.finalizationReserveMs
    );
    ...
    const opportunity = evaluateCoronationElsaSettleOpportunity({
      ...
      readinessWaitMs: CORONATION_ELSA_PLANNER_CONFIG.boardTraceReadinessWaitMaxMs,
      recoveryWaitMs: CORONATION_ELSA_PLANNER_CONFIG.traceRecoveryWaitMaxMs,
      stablePhysicsTicks: CORONATION_ELSA_PLANNER_CONFIG.tracePotentialStablePhysicsTicks,
      cycle: this.strongestModeCoronationElsaSettleOpportunityCycle
    });
    ...
    this.strongestModeCoronationElsaSettleOpportunityCycle = Object.freeze({
      ...
      futureTraceRelevantPendingCount: Math.max(0, basePlan.diagnostics?.futureTraceRelevantPendingCount || 0)
    });
    this.recordStrongestModeCoronationElsaOpportunityDecision(opportunity, basePlan);
    if (![STRONGEST_MODE_CORONATION_ELSA_SETTLE_OPPORTUNITY_WAIT_REASON, STRONGEST_MODE_CORONATION_ELSA_PRE_TAP_SETTLE_WAIT_REASON, STRONGEST_MODE_CORONATION_ELSA_BOARD_TRACE_READINESS_WAIT_REASON, STRONGEST_MODE_CORONATION_ELSA_TRACE_RECOVERY_WAIT_REASON].includes(opportunity.plan.waitReason)) {
    ...
    const validation = chain.length >= 3
      ? this.validateStrongestModeCoronationElsaPlannedChain(chain, { context })
      : null;
    const validationMs = clock() - validationStartedAt;
    const totalPlanActionMs = clock() - startedAt;
    this.strongestModeCoronationElsaPlannerBlockedTotalMs += Math.max(0, totalPlanActionMs);
    plan = Object.freeze({
      ...plan,
      diagnostics: Object.freeze({
        ...(plan.diagnostics || {}),
        ...
        validationMs,
        totalPlanActionMs,
        plannerCallsThisFrame: this.strongestModeCoronationElsaPlannerFrameCallCount,
        over4ms: totalPlanActionMs > 4,
        over8ms: totalPlanActionMs > 8,
        over16ms: totalPlanActionMs > 16
      })
    });
    if (chain.length >= 3) {
      Object.defineProperty(chain, "strongestModeCoronationElsaPlan", {
      ...
      Object.defineProperty(chain, "strongestModeCoronationElsaValidationToken", {
        value: Object.freeze({ context, revision: context.revision, validation }),
        configurable: true
      });
    }
  ...
  resetStrongestModeCoronationElsaSettleOpportunityState() {
    ...
    this.strongestModeCoronationElsaOpportunityPerfBaseline = null;
    this.strongestModeCoronationElsaTemporalPositions = new Map();
    this.strongestModeCoronationElsaFinalTraceWait = null;
    this.strongestModeCoronationElsaSettleOpportunityCycle = Object.freeze({
      readinessUsed: false,
      recoveryUsed: false,
      finalTraceWaitUsed: false,
      traceCount: 0
    });
  }
  ...
  resetStrongestModeCoronationElsaIceTapReadiness() {
    this.strongestModeCoronationElsaIceTapReadinessState = null;
    this.strongestModeCoronationElsaIceTapReadinessCache = null;
  }

  shouldWaitForStrongestModeCoronationElsaFinalTrace(chain) {
    const plan = chain?.strongestModeCoronationElsaPlan || null;
    const committedTraceCount = this.strongestModeCoronationElsaSettleOpportunityCycle?.traceCount || 0;
    const finalTraceThreshold = Math.max(1, (this.strongestModeCoronationElsaMinimumTraceCount || 4) - 1);
    if (
      !plan || plan.action !== "trace" || committedTraceCount < finalTraceThreshold ||
      this.strongestModeCoronationElsaSettleOpportunityCycle?.finalTraceWaitUsed ||
      Math.max(0, plan.maxAdditionalTraces || 0) > 1
    ) {
      this.strongestModeCoronationElsaFinalTraceWait = null;
      return false;
    }
    const token = chain.strongestModeCoronationElsaValidationToken || null;
    const context = token?.context;
    if (!context?.snapshot || !this.isCoronationElsaPlannerRevisionCurrent(token.revision)) return false;
    const chainIndices = chain.map((node) => getCoronationElsaPlannerNodeIndex(context.snapshot, node.id));
    if (chainIndices.some((index) => index < 0)) return false;
    const risk = evaluateCoronationElsaFinalTraceSettleRisk(context.snapshot, context.initialState, chainIndices);
    const physicsStepCount = this.strongestModeCoronationElsaPhysicsStepCount || 0;
    const signature = `${token.revision.freezeRevision}:${plan.chainIds.join(",")}`;
    const pending = this.strongestModeCoronationElsaFinalTraceWait;
    if (pending) {
      if (physicsStepCount <= pending.startedPhysicsStepCount) return true;
      const summary = this.getStrongestModeCoronationElsaSkillSummary?.();
      if (summary) {
        summary.finalTracePreWaitTotalMs += Math.max(0, ((this.elapsed || 0) * 1000) - pending.startedAtMs);
        summary.finalTracePreWaitPhysicsTicks += Math.max(0, physicsStepCount - pending.startedPhysicsStepCount);
        summary.finalTracePreWaitFreezeTargetGain += risk.freezeTargetCount - pending.freezeTargetCount;
      }
      this.strongestModeCoronationElsaFinalTraceWait = null;
      return false;
    }
    if (!risk.shouldWait) return false;
    this.strongestModeCoronationElsaFinalTraceWait = Object.freeze({
      signature,
      startedPhysicsStepCount: physicsStepCount,
      startedAtMs: Math.max(0, this.elapsed || 0) * 1000,
      freezeTargetCount: risk.freezeTargetCount
    });
    this.strongestModeCoronationElsaSettleOpportunityCycle = Object.freeze({
      ...this.strongestModeCoronationElsaSettleOpportunityCycle,
      finalTraceWaitUsed: true
    });
    this.strongestModeStepTimer = this.strongestModeStepInterval;
    const summary = this.getStrongestModeCoronationElsaSkillSummary?.();
    if (summary) {
      summary.finalTracePreWaitCount += 1;
      summary.finalTraceRelatedUnsettledCount = risk.relatedUnsettledCount;
      summary.finalTraceMaxVelocity = risk.maxVelocity;
      summary.finalTraceMaxPositionDelta = risk.maxPositionDelta;
    }
    return true;
  }
  ...
  evaluateStrongestModeCoronationElsaIceTapReadiness(specialTarget) {
    ...
    this.strongestModeCoronationElsaIceTapReadinessCache = Object.freeze({ key: cacheKey, result });
    const summary = this.getStrongestModeCoronationElsaSkillSummary?.();
    if (summary) {
      ...
      if (ready && summary.iceTapReadyPhysicsStepCount == null) {
        summary.iceTapReadyPhysicsStepCount = physicsStepCount;
        summary.iceTapReadyElapsedSec = Math.max(0, this.elapsed || 0);
      }
    }
  ...
  updateStrongestModeCoronationElsaSettleWaveFromPlan(plan) {
    ...
    const futurePending = Math.max(0, plan?.diagnostics?.futureTraceRelevantPendingCount || 0);
    const settling = Math.max(0, plan?.diagnostics?.settlingOpportunityNodeCount || 0);
    const recent = Math.max(0, plan?.diagnostics?.recentSpawnCount || 0);
    if (activeInflowNodeCount === 0 && futurePending === 0 && settling === 0 && recent === 0 && !this.strongestModeCoronationElsaOpportunityEpisode) {
      this.strongestModeCoronationElsaSettleWaveOpen = false;
      return;
    }
    if ((activeInflowNodeCount > 0 || futurePending > 0 || settling > 0 || recent > 0) && this.strongestModeCoronationElsaSettleWaveId <= 0) {
  ...
  validateStrongestModeCoronationElsaPlannedChain(chain, options = {}) {
    ...
    const token = chain.strongestModeCoronationElsaValidationToken || null;
    if (token?.validation && this.isCoronationElsaPlannerRevisionCurrent(token.revision)) {
      return token.validation;
    }
    const suppliedContext = options.context || token?.context || null;
    const context = suppliedContext && this.isCoronationElsaPlannerRevisionCurrent(suppliedContext.revision)
      ? suppliedContext
      : this.buildCoronationElsaPlannerContext(options);
    const chainIndices = chain.map((node) => getCoronationElsaPlannerNodeIndex(context.snapshot, node.id));
    ...
    if (isCoronationElsaSkillActive) {
      const executeDecision = (decision, allowReplan) => {
        ...
        if (decision.plan.action === "trace") {
          ...
          if (this.shouldWaitForStrongestModeCoronationElsaFinalTrace?.(decision.chain)) return false;
          const traceCountBefore = this.strongestModeCoronationElsaSettleOpportunityCycle?.traceCount || 0;
          ...
          const summary = this.getStrongestModeCoronationElsaSkillSummary?.();
          if (chained && summary && traceCountBefore === 0 && !summary.firstTraceBoardReadiness) {
  ...
  getStrongestModeCoronationElsaFlowSafetyContext() {
    ...
    const physicsBodies = Object.freeze([...this.getPhysicsBodies()]);
    return Object.freeze({
      safePlayableY,
      ...
      physicsBodies,
      supportStateByBody: buildStrongestModeCoronationElsaFlowSupportStates(this, physicsBodies)
    });
  }
  ...
  tryTapStrongestModeCoronationElsaFreezeTarget(specialTarget, options = {}) {
    ...
    const summary = this.getStrongestModeCoronationElsaSkillSummary?.();
    if (summary) {
      summary.tapPendingGeometryCount = Math.max(0, this.strongestModeCoronationElsaSettleOpportunityCycle?.pendingGeometryNodeCount || 0);
      summary.tapFutureTraceRelevantPendingCount = Math.max(0, this.strongestModeCoronationElsaSettleOpportunityCycle?.futureTraceRelevantPendingCount || 0);
      summary.tracesBeforeTap = Math.max(0, this.strongestModeCoronationElsaSettleOpportunityCycle?.traceCount || 0);
      const physicsStepCount = this.strongestModeCoronationElsaPhysicsStepCount || 0;
      summary.finalTraceCommitToTapPhysicsTicks = summary.finalTraceCommitPhysicsStepCount == null
        ? null
        : Math.max(0, physicsStepCount - summary.finalTraceCommitPhysicsStepCount);
      summary.finalTraceCommitToTapMs = summary.finalTraceCommitElapsedSec == null
        ? null
        : Math.max(0, ((this.elapsed || 0) - summary.finalTraceCommitElapsedSec) * 1000);
      summary.iceTapReadyToTapPhysicsTicks = summary.iceTapReadyPhysicsStepCount == null
        ? null
        : Math.max(0, physicsStepCount - summary.iceTapReadyPhysicsStepCount);
    }
    this.strongestModeCoronationElsaNoChainFrames = 0;
    this.strongestModeCoronationElsaNoTraceDurationSec = 0;
    this.strongestModeCoronationElsaStopLogged = false;
  ...
  beginStrongestModeCoronationElsaSkillSummary(sessionId) {
    ...
    this.strongestModeCoronationElsaSkillSummary = {
      sessionId,
      skillStartedElapsedSec: Math.max(0, this.elapsed || 0),
      chainCount: 0,
      totalChainLength: 0,
      maxChainLength: 0,
      freezeApplyCount: 0,
      ...
      plannerExactSearchSamplesMs: [],
      plannerBeamSearchSamplesMs: [],
      plannerTotalPlanActionSamplesMs: [],
      plannerFlowSupportBuildSamplesMs: [],
      plannerSnapshotBuildSamplesMs: [],
      plannerAdjacencyBuildSamplesMs: [],
      plannerSolveSamplesMs: [],
      plannerValidationSamplesMs: [],
      plannerOver4msCount: 0,
      plannerOver8msCount: 0,
      plannerOver16msCount: 0,
      plannerSupportCacheHitCount: 0,
      plannerAdjacencyCacheHitCount: 0,
      plannerCallsPerFrameMax: 0,
      traceExecutionSamplesMs: [],
      traceExecutionTotalMs: 0,
      traceExecutionMaxMs: 0,
      plannerCallsPerTraceSamples: [],
      ...
      plannerRootLegalTraceCandidateCount: 0,
      plannerRootSafeTraceCandidateCount: 0,
      plannerRootFlowBlockedTraceCandidateCount: 0,
      plannerFutureTemporarilyUnsafeCandidateCount: 0,
      plannerInflowUnsafeNodeCount: 0,
      plannerUpperInflowNodeCount: 0,
      plannerRecentSpawnCount: 0,
      plannerUnsettledNodeCount: 0,
      plannerStableSupportNodeCount: 0,
      plannerDynamicSupportNodeCount: 0,
      plannerGenuineFallSpaceNodeCount: 0,
      plannerSettlingOpportunityNodeCount: 0,
      plannerPendingGeometryNodeCount: 0,
      plannerStableSupportButUnsettledCount: 0,
      plannerSettlingOpportunityAboveSelectionCount: 0,
      plannerActiveInflowAboveSelectionCount: 0,
      plannerPendingGeometryAboveSelectionCount: 0,
      plannerSelectedCandidateVerticalSpan: 0,
      ...
      boardTraceReadinessWaitCount: 0,
      traceRecoveryWaitCount: 0,
      traceRecoveryTraceRecoveredCount: 0,
      traceRecoveryTapCount: 0,
      traceWaitEvents: [],
      tapFutureTraceRelevantPendingCount: 0,
      totalSettleWaitMs: 0,
      waitBudgetRemainingMs: CORONATION_ELSA_PLANNER_CONFIG.opportunityCycleWaitBudgetMs,
      tapPendingGeometryCount: 0,
      tracesBeforeTap: 0,
      finalTracePreWaitCount: 0,
      finalTracePreWaitTotalMs: 0,
      finalTracePreWaitPhysicsTicks: 0,
      finalTracePreWaitFreezeTargetGain: 0,
      finalTraceRelatedUnsettledCount: 0,
      finalTraceMaxVelocity: 0,
      finalTraceMaxPositionDelta: 0,
      finalTraceCommitPhysicsStepCount: null,
      finalTraceCommitElapsedSec: null,
      iceTapReadyPhysicsStepCount: null,
      iceTapReadyElapsedSec: null,
      finalTraceCommitToTapPhysicsTicks: null,
      finalTraceCommitToTapMs: null,
      iceTapReadyToTapPhysicsTicks: null,
      iceTapReady: false,
      ...
      opportunityWaitPhysicsStepSamples: [],
  ...
  recordStrongestModeCoronationElsaPlannerRun(plan) {
    ...
    if (plan.mode === "beam") summary.plannerBeamSearchSamplesMs.push(Math.max(0, diagnostics.searchTimeMs || 0));
    ...
    summary.plannerTotalPlanActionSamplesMs.push(Math.max(0, diagnostics.totalPlanActionMs || 0));
    summary.plannerFlowSupportBuildSamplesMs.push(Math.max(0, diagnostics.flowSupportBuildMs || 0));
    summary.plannerSnapshotBuildSamplesMs.push(Math.max(0, diagnostics.snapshotBuildMs || 0));
    summary.plannerAdjacencyBuildSamplesMs.push(Math.max(0, diagnostics.adjacencyBuildMs || 0));
    summary.plannerSolveSamplesMs.push(Math.max(0, diagnostics.solveMs || diagnostics.searchTimeMs || 0));
    summary.plannerValidationSamplesMs.push(Math.max(0, diagnostics.validationMs || 0));
    if ((diagnostics.totalPlanActionMs || 0) > 4) summary.plannerOver4msCount += 1;
    if ((diagnostics.totalPlanActionMs || 0) > 8) summary.plannerOver8msCount += 1;
    if ((diagnostics.totalPlanActionMs || 0) > 16) summary.plannerOver16msCount += 1;
    if (diagnostics.supportCacheHit) summary.plannerSupportCacheHitCount += 1;
    if (diagnostics.adjacencyCacheHit) summary.plannerAdjacencyCacheHitCount += 1;
    summary.plannerCallsPerFrameMax = Math.max(
      summary.plannerCallsPerFrameMax,
      diagnostics.plannerCallsThisFrame || 0
    );
    ...
    summary.plannerRootLegalTraceCandidateCount = diagnostics.rootLegalTraceCandidateCount || 0;
    summary.plannerRootSafeTraceCandidateCount = diagnostics.rootSafeTraceCandidateCount || 0;
    summary.plannerRootFlowBlockedTraceCandidateCount = diagnostics.rootFlowBlockedTraceCandidateCount || 0;
    summary.plannerFutureTemporarilyUnsafeCandidateCount += Math.max(0, diagnostics.futureTemporarilyUnsafeCandidateCount || 0);
    ...
    summary.plannerStableSupportNodeCount = diagnostics.stableSupportNodeCount || 0;
    summary.plannerDynamicSupportNodeCount = diagnostics.dynamicSupportNodeCount || 0;
    summary.plannerGenuineFallSpaceNodeCount = diagnostics.genuineFallSpaceNodeCount || 0;
    summary.plannerSettlingOpportunityNodeCount = diagnostics.settlingOpportunityNodeCount || 0;
    summary.plannerPendingGeometryNodeCount = diagnostics.pendingGeometryNodeCount || 0;
    summary.plannerStableSupportButUnsettledCount = diagnostics.stableSupportButUnsettledCount || 0;
    summary.plannerSettlingOpportunityAboveSelectionCount = diagnostics.settlingOpportunityAboveSelectionCount || 0;
    summary.plannerActiveInflowAboveSelectionCount = diagnostics.activeInflowAboveSelectionCount || 0;
    summary.plannerPendingGeometryAboveSelectionCount = diagnostics.pendingGeometryAboveSelectionCount || 0;
    summary.plannerSelectedCandidateVerticalSpan = diagnostics.selectedCandidateVerticalSpan || 0;
  ...
  recordStrongestModeCoronationElsaPlannerDecision(plan) {
    ...
    if (plan.action === "wait") {
      if (!Number.isFinite(this.strongestModeCoronationElsaInflowWaitStartedAt)) {
        ...
        this.strongestModeCoronationElsaInflowWaitReason = plan.waitReason || "WAIT_FOR_INFLOW";
        const summary = this.getStrongestModeCoronationElsaSkillSummary();
        if (summary) {
          if (this.strongestModeCoronationElsaInflowWaitReason === "WAIT_FOR_BOARD_REFILL") {
            summary.waitForBoardRefillCount += 1;
          } else {
            summary.waitForInflowCount += 1;
          }
        }
    ...
    const waitReason = this.strongestModeCoronationElsaInflowWaitReason || "WAIT_FOR_INFLOW";
    const summary = this.getStrongestModeCoronationElsaSkillSummary();
    if (summary) {
      ...
      if (plan.action === "trace" && waitReason === "WAIT_FOR_INFLOW") {
        summary.waitForInflowToSafeTraceCount += 1;
      }
    }
    if (this.coronationElsaDebug !== false) {
      this.logCodexCoronationPayload("[CODEXLOG CORONATION WAIT FOR INFLOW]", {
  ...
  recordStrongestModeCoronationElsaOpportunityDecision(result, basePlan) {
    ...
    if (result.event.type === "start") {
      this.strongestModeCoronationElsaOpportunitySuppressionRecordedWaveId = null;
      this.strongestModeCoronationElsaOpportunityPerfBaseline = Object.freeze({
        wallClockMs: coronationElsaPlannerNowMs(),
        gameplayDeltaSec: this.strongestModeCoronationElsaGameplayDeltaTotalSec || 0,
        plannerBlockedMs: this.strongestModeCoronationElsaPlannerBlockedTotalMs || 0,
        physicsStepCount: this.strongestModeCoronationElsaPhysicsStepCount || 0
      });
      if (summary) {
        summary.opportunityWaitCount += 1;
        if (result.event.kind === "readiness") summary.boardTraceReadinessWaitCount += 1;
        if (result.event.kind === "recovery") summary.traceRecoveryWaitCount += 1;
        summary.traceWaitEvents ||= [];
        summary.traceWaitEvents.push({
          phase: "start",
          kind: result.event.kind,
          maxAdditionalTraces: result.event.baselineMaxAdditionalTraces,
          futureTraceRelevantPendingCount: basePlan?.diagnostics?.futureTraceRelevantPendingCount || 0
        });
      }
      if (this.coronationElsaDebug !== false) {
    ...
    if (summary) {
      ...
      if (result.event.kind === "recovery") {
        if (result.plan?.action === "trace") summary.traceRecoveryTraceRecoveredCount += 1;
        if (result.plan?.action === "tap") summary.traceRecoveryTapCount += 1;
      }
      summary.traceWaitEvents ||= [];
      summary.traceWaitEvents.push({
        phase: "release",
        kind: result.event.kind,
        releaseReason: result.event.releaseReason,
        maxAdditionalTracesBefore: result.event.baselineMaxAdditionalTraces,
        maxAdditionalTracesAfter: result.event.currentMaxAdditionalTraces,
      ...
      const baseline = this.strongestModeCoronationElsaOpportunityPerfBaseline;
      if (baseline) {
        summary.opportunityWaitWallClockSamplesMs ||= [];
        summary.opportunityWaitGameplayDeltaSamplesMs ||= [];
        summary.opportunityWaitPlannerBlockedSamplesMs ||= [];
        summary.opportunityWaitPhysicsStepSamples ||= [];
        summary.opportunityWaitWallClockSamplesMs.push(Math.max(
          0,
          coronationElsaPlannerNowMs() - baseline.wallClockMs
        ));
        summary.opportunityWaitGameplayDeltaSamplesMs.push(Math.max(
          0,
          ((this.strongestModeCoronationElsaGameplayDeltaTotalSec || 0) - baseline.gameplayDeltaSec) * 1000
        ));
        summary.opportunityWaitPlannerBlockedSamplesMs.push(Math.max(
          0,
          (this.strongestModeCoronationElsaPlannerBlockedTotalMs || 0) - baseline.plannerBlockedMs
        ));
        summary.opportunityWaitPhysicsStepSamples.push(Math.max(
          0,
          (this.strongestModeCoronationElsaPhysicsStepCount || 0) - baseline.physicsStepCount
        ));
      }
    }
    this.strongestModeCoronationElsaOpportunityPerfBaseline = null;
  ...
  emitStrongestModeCoronationElsaSkillSummary(endedBy) {
    ...
    const summarizeMs = (samples = []) => {
      if (!samples.length) return Object.freeze({ avg: 0, median: 0, p95: 0, max: 0 });
    ...
    const summaryPayload = {
      ...
      plannerExactSearchMs: summarizeMs(summary.plannerExactSearchSamplesMs),
      plannerBeamSearchMs: summarizeMs(summary.plannerBeamSearchSamplesMs),
      totalPlanActionMs: summarizeMs(summary.plannerTotalPlanActionSamplesMs),
      flowSupportBuildMs: summarizeMs(summary.plannerFlowSupportBuildSamplesMs),
      snapshotBuildMs: summarizeMs(summary.plannerSnapshotBuildSamplesMs),
      adjacencyBuildMs: summarizeMs(summary.plannerAdjacencyBuildSamplesMs),
      solveMs: summarizeMs(summary.plannerSolveSamplesMs),
      validationMs: summarizeMs(summary.plannerValidationSamplesMs),
      traceExecutionMs: summarizeMs(summary.traceExecutionSamplesMs),
      plannerOver4msCount: summary.plannerOver4msCount,
      plannerOver8msCount: summary.plannerOver8msCount,
      plannerOver16msCount: summary.plannerOver16msCount,
      plannerCallsPerFrameMax: summary.plannerCallsPerFrameMax,
      plannerCallsPerTrace: summarizeMs(summary.plannerCallsPerTraceSamples),
      plannerSupportCacheHitRate: summary.plannerRunCount > 0
        ? Number((summary.plannerSupportCacheHitCount / summary.plannerRunCount).toFixed(3))
        : 0,
      plannerAdjacencyCacheHitRate: summary.plannerRunCount > 0
        ? Number((summary.plannerAdjacencyCacheHitCount / summary.plannerRunCount).toFixed(3))
        : 0,
      opportunityWaitWallClockMs: summarizeMs(summary.opportunityWaitWallClockSamplesMs),
      opportunityWaitGameplayDeltaMs: summarizeMs(summary.opportunityWaitGameplayDeltaSamplesMs),
      opportunityWaitPlannerBlockedMs: summarizeMs(summary.opportunityWaitPlannerBlockedSamplesMs),
      opportunityWaitPhysicsUpdates: summarizeMs(summary.opportunityWaitPhysicsStepSamples),
      ...
      plannerRootLegalTraceCandidateCount: summary.plannerRootLegalTraceCandidateCount,
      plannerRootSafeTraceCandidateCount: summary.plannerRootSafeTraceCandidateCount,
      plannerRootFlowBlockedTraceCandidateCount: summary.plannerRootFlowBlockedTraceCandidateCount,
      plannerFutureTemporarilyUnsafeCandidateCount: summary.plannerFutureTemporarilyUnsafeCandidateCount,
      ...
      plannerStableSupportNodeCount: summary.plannerStableSupportNodeCount,
      plannerDynamicSupportNodeCount: summary.plannerDynamicSupportNodeCount,
      plannerGenuineFallSpaceNodeCount: summary.plannerGenuineFallSpaceNodeCount,
      ...
      waitForInflowAverageDurationSec: (summary.waitForInflowCount + summary.waitForBoardRefillCount) > 0
        ? Number((summary.waitForInflowTotalDurationSec / (
          summary.waitForInflowCount + summary.waitForBoardRefillCount
        )).toFixed(3))
        : 0,
      ...
      boardTraceReadinessWaitCount: summary.boardTraceReadinessWaitCount,
      traceRecoveryWaitCount: summary.traceRecoveryWaitCount,
      traceRecoveryTraceRecoveredCount: summary.traceRecoveryTraceRecoveredCount,
      traceRecoveryTapCount: summary.traceRecoveryTapCount,
      ...
      settlingOpportunityAboveSelectionCount: summary.plannerSettlingOpportunityAboveSelectionCount,
      pendingGeometryNodeCount: summary.plannerPendingGeometryNodeCount,
      pendingGeometryAboveSelectionCount: summary.plannerPendingGeometryAboveSelectionCount,
      stableSupportButUnsettledCount: summary.plannerStableSupportButUnsettledCount,
      ...
      finalTracePreWaitCount: summary.finalTracePreWaitCount,
      finalTracePreWaitTotalMs: Number(summary.finalTracePreWaitTotalMs.toFixed(3)),
      finalTracePreWaitPhysicsTicks: summary.finalTracePreWaitPhysicsTicks,
      finalTracePreWaitFreezeTargetGain: summary.finalTracePreWaitFreezeTargetGain,
      finalTraceRelatedUnsettledCount: summary.finalTraceRelatedUnsettledCount,
      finalTraceMaxVelocity: Number(summary.finalTraceMaxVelocity.toFixed(3)),
      finalTraceMaxPositionDelta: Number(summary.finalTraceMaxPositionDelta.toFixed(3)),
      finalTraceCommitToTapPhysicsTicks: summary.finalTraceCommitToTapPhysicsTicks,
      finalTraceCommitToTapMs: summary.finalTraceCommitToTapMs == null ? null : Number(summary.finalTraceCommitToTapMs.toFixed(3)),
      ...
      tracesPerTap: summary.endedBy === "iceTap" ? summary.chainCount : null,
      oneTraceThenTap: summary.endedBy === "iceTap" && summary.chainCount === 1,
      fourOrMoreTraces: summary.endedBy === "iceTap" && summary.chainCount >= 4,
      actualIceTapEffectiveClear: summary.actualIceTapEffectiveClear,
      actualIceTapRawCoins: summary.actualIceTapRawCoins,
      skillDurationSec: Number(Math.max(0, (this.elapsed || 0) - summary.skillStartedElapsedSec).toFixed(3)),
      rawCoinsPerGameSecond: (this.elapsed || 0) > summary.skillStartedElapsedSec
        ? Number((summary.actualIceTapRawCoins / ((this.elapsed || 0) - summary.skillStartedElapsedSec)).toFixed(3))
        : 0,
    ...
    if (this.coronationElsaPerf && typeof document !== "undefined") {
      let output = document.getElementById("coronationElsaPerfResults");
      if (!output) {
        output = document.createElement("script");
        output.id = "coronationElsaPerfResults";
        output.type = "application/json";
        output.hidden = true;
        document.body.appendChild(output);
      }
      const samples = output.textContent ? JSON.parse(output.textContent) : [];
      const perfSample = {
        ...summaryPayload,
        rawSamples: {
          totalPlanActionMs: summary.plannerTotalPlanActionSamplesMs.slice(),
          exactSearchMs: summary.plannerExactSearchSamplesMs.slice(),
          beamSearchMs: summary.plannerBeamSearchSamplesMs.slice(),
          flowSupportBuildMs: summary.plannerFlowSupportBuildSamplesMs.slice(),
          snapshotBuildMs: summary.plannerSnapshotBuildSamplesMs.slice(),
          adjacencyBuildMs: summary.plannerAdjacencyBuildSamplesMs.slice(),
          solveMs: summary.plannerSolveSamplesMs.slice(),
          validationMs: summary.plannerValidationSamplesMs.slice(),
          traceExecutionMs: summary.traceExecutionSamplesMs.slice(),
          plannerCallsPerTrace: summary.plannerCallsPerTraceSamples.slice(),
          opportunityWaitWallClockMs: summary.opportunityWaitWallClockSamplesMs.slice(),
          opportunityWaitGameplayDeltaMs: summary.opportunityWaitGameplayDeltaSamplesMs.slice(),
          opportunityWaitPlannerBlockedMs: summary.opportunityWaitPlannerBlockedSamplesMs.slice(),
          opportunityWaitPhysicsUpdates: summary.opportunityWaitPhysicsStepSamples.slice(),
          traceWaitEvents: summary.traceWaitEvents.slice()
        }
      };
      if (samples.length < this.coronationElsaPerfTarget) samples.push(perfSample);
      output.textContent = JSON.stringify(samples);
      output.dataset.count = String(samples.length);
      output.dataset.target = String(this.coronationElsaPerfTarget);
    }
    if (this.coronationElsaDebug) {
  ...
  performStrongestModeChains(initialChain = [], options = {}) {
    ...
    while (performed < maxChainsThisStep) {
      ...
      if (chainSucceeded && isCoronationElsaSkillActive) {
        ...
        this.strongestModeCoronationElsaSettleOpportunityCycle = Object.freeze({
          ...cycle,
          traceCount: Math.max(0, cycle.traceCount || 0) + 1
        });
        this.resetStrongestModeCoronationElsaIceTapReadiness?.();
        if (coronationElsaSummary) {
          coronationElsaSummary.finalTraceCommitPhysicsStepCount = this.strongestModeCoronationElsaPhysicsStepCount || 0;
          coronationElsaSummary.finalTraceCommitElapsedSec = Math.max(0, this.elapsed || 0);
          coronationElsaSummary.iceTapReadyPhysicsStepCount = null;
          coronationElsaSummary.iceTapReadyElapsedSec = null;
        }
      }
      if (!chainSucceeded) {
        break;
  ...
  performStrongestModeChain(chain, options = {}) {
    const traceExecutionStartedAt = coronationElsaPlannerNowMs();
    const allowChainQueueDuringActiveClear = !!options.allowChainQueueDuringActiveClear;
    const canAttemptChain = (
      !this.isStrongestModeBusy() ||
      (allowChainQueueDuringActiveClear && this.canQueueChainDuringActiveClear())
    ...
    if (
      ...
      const coronationElsaChainStartLog = {
        ...
        plannerMode: chain.strongestModeCoronationElsaPlan?.mode || null,
        projectedTraces: chain.strongestModeCoronationElsaPlan?.maxAdditionalTraces || 0,
        totalPlanActionMs: chain.strongestModeCoronationElsaPlan?.diagnostics?.totalPlanActionMs || 0,
    ...
    this.finishChain();
    if (coronationElsaSummary) {
      const traceExecutionMs = Math.max(0, coronationElsaPlannerNowMs() - traceExecutionStartedAt);
      coronationElsaSummary.traceExecutionSamplesMs.push(traceExecutionMs);
      coronationElsaSummary.traceExecutionTotalMs += traceExecutionMs;
      coronationElsaSummary.traceExecutionMaxMs = Math.max(
        coronationElsaSummary.traceExecutionMaxMs,
        traceExecutionMs
      );
      coronationElsaSummary.plannerCallsPerTraceSamples.push(
        this.strongestModeCoronationElsaPlannerCallsSinceTrace
      );
      this.strongestModeCoronationElsaPlannerLastTraceCallCount = this.strongestModeCoronationElsaPlannerCallsSinceTrace;
      this.strongestModeCoronationElsaPlannerCallsSinceTrace = 0;
    }
  ...
  finishRun() {
    ...
    this.state = "result";
    const perfSampleCount = this.coronationElsaPerf && typeof document !== "undefined"
      ? Number(document.getElementById("coronationElsaPerfResults")?.dataset?.count || 0)
      : 0;
    const shouldRestartCoronationElsaPerf = !!(
      this.coronationElsaPerf && perfSampleCount < this.coronationElsaPerfTarget
    );
    const willAutoRestart = (
      (this.aiAutoPlay && !this.aiLearningMode) ||
      this.shouldAiLearningAutoRepeat() ||
      shouldRestartCoronationElsaPerf
    );
    if (!willAutoRestart) {
      this.itemSelection = this.blankItemSelection();
    }
    ...
    } else if (shouldRestartCoronationElsaPerf) {
      this.clearAiLearningRestartTimer();
      this.aiLearningRestartTimer = setTimeout(() => {
        this.aiLearningRestartTimer = null;
        if (this.coronationElsaPerf && this.state === "result") {
          this.startGame({ skipProgressSave: true });
          this.disableAiModesForStrongestMode();
          this.strongestModeEnabled = true;
        }
